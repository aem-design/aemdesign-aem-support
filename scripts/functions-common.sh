...
function createFileFromTemplate() {
    local SEARCH_SED_REGEX=${1?Need SED Replace Regex}
    local TEMPLATE_FILE=${2?Need Template file}
    local OUTPUT_FILE=${3?Need Output file}

#    sed "'$SEARCH_SED_REGEX'" "$OUTPUT_FILE" | diff -p "$OUTPUT_FILE" - | patch

    sed "$SEARCH_SED_REGEX" "$TEMPLATE_FILE" | tee "$OUTPUT_FILE"
