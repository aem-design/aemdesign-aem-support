...
const args = require('yargs')
  ...
  .example(
    'node generator.js --config=<filename>.yml',
    'run conversion of config',
  )
  .example(
    'node generator.js --config=<filename>.yml --debug',
    'run with debug enabled',
  )
  .example(
    'node generator.js --config=<filename>.yml --no-clean',
    'run without tree cleaning',
  )
  .example(
    'node generator.js --config=<filename>.yml --no-console',
    'run without realtime logging',
  )
...
try {
  ...
  const configFilePath = currentPath(
    `${tmpPath}/config.${(Date.now() / 1000) | 0}.yml`,
  )
  ...
  function generator() {
    ...
    for (const category of Object.keys(config)) {
      ...
      for (const subcategory of Object.keys(children)) {
        ...
        categories[category][subcategory] = categories[category][
          subcategory
        ] || {
          content: [],
        }
        ...
        if (prefixes !== undefined) {
          for (const index in prefixes) {
            ...
            if (typeof prefix === 'object') {
              ...
              //get title
              prefixTitle = prefixConfig['title']
                ? prefixConfig['title']
                : prefix
            ...
            let prefixToFolders = prefixIsAdvanced
              ? { name: prefix }
              : data.prefixToFolders || {}
            ...
            if (isArray(data.breakpoints)) {
                ...
                if (isArray(data.sizes)) {
                  for (const size of data.sizes) {
                    ...
                    categories[category][subcategory].content.push({
                      ...
                      prefixValue: prefixValue,
                      template: data.template || '',
                      title: parseTitle(data.title, prefix.toString(), {
                        bp,
                        emptyTitle,
                        size,
                      }),
              ...
              for (const size of data.sizes) {
                ...
                let valueFormatted
                if (valueFormat) {
                  valueFormatted = valueFormat.replace(
                    new RegExp('%%prefix%%', 'g'),
                    prefix,
                  )
                } else {
              ...
              if (valueFormat) {
                valueFormatted = valueFormat.replace(
                  new RegExp('%%prefix%%', 'g'),
                  prefixIsAdvanced ? prefixValue : value,
                )
              } else {
                valueFormatted = prefixValue
                  ? prefixValue.concat(prefix)
              ...
              categories[category][subcategory].content.push({
                ...
                prefixValue: prefixValue,
                template: data.template || '',
                title: prefixIsAdvanced
                  ? prefixTitle
                  : parseTitle(data.title, prefix.toString(), { emptyTitle }),
                type: 'tag',
                value: value,
                valueFormatted: valueFormatted,
              })
    ...
    Object.keys(categories).forEach((category) => {
      ...
      if (category.startsWith(pathPrefixTags)) {
        ...
        categoryTemplateDefault =
          tagParentTemplate || loadTemplateForCategory('tag-parent')
      } else if (category.startsWith(pathPrefixApps)) {
        categoryTemplate = contentTemplate || loadTemplateForCategory('content')
        categoryTemplateDefault =
          contentTemplate || loadTemplateForCategory('content-parent')
      } else {
        return
      }
    ...
    console.log(
      colors.blue('Total number of files generated:'),
      totalNumberOfTags,
    )
  }
